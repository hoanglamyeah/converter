    ...
    <div class="container">
        <nav class="breadcrumb has-arrow-separator" aria-label="breadcrumbs">
            <ul>
                ...
                <li>
                    <nuxt-link :to="'/'+ type">{{ typeName }}</nuxt-link>
                </li>
                <li>
                    <nuxt-link :to="'/'+ type + '/unit'">Unit</nuxt-link>
                </li>
                <li class="is-active">
                    <nuxt-link :to="'/'+ type + '/unit/'+unit.id" aria-current="page">{{unit.name}}
                        Converter
                    </nuxt-link>
                </li>
            </ul>
                ...
                <section class="has-text-left">
                    ...
                    <table class="convert-list table is-bordered is-striped is-narrow is-hoverable is-fullwidth">
                        ...
                        <tr v-for="convert in converts" v-bind:key="convert.id">
                            <td>
                                <nuxt-link :to="'/'+type+'/' + convert[0].id + '-to-' + convert[1].id">{{
                                    convert[0].name
                                    }} to {{
                                    convert[1].name }}
                                </nuxt-link>
...
<script>
    ...
    export default {
        ...
        computed: {
            converts() {
                var arr = []
                var all = ultil.getAll(this.type)
            ...
            unit() {
                return ultil.getUnit(this.type, this.$route.params.slug)
            },
